...
layout = [
            [sg.Frame(layout=[[sg.InputText('Set The Directory to compress all tiffs in',
             ...
             sg.B('+',key="-FONT_BIGGER-"),sg.B('-',key="-FONT_SMALLER-",font=('',str(fontSize)))],
            [sg.T("Degree of Compression \n(0=None, 9 max, recommend 1)",key='fred',text_color='#ccaaea',font=('',str(fontSize))),
             sg.Slider(range=(0,9),default_value=1,orientation='h',key='-COMPRESSION_AMOUNT-',font=('',str(fontSize)),text_color="orange",trough_color='#556088')],
         ]
...
while True:             
    ...
    if event in (sg.WIN_CLOSED, 'Cancel'):
        break
    if event=='-FONT_BIGGER-':
        fontSize=fontSize+1
        print('size plus 1,'+str(fontSize))
        fSize=str(fontSize)
        window['fred'].update(font=("any",fSize))
        # window['-FILE_LOCATION-'].update(font=("any",fSize))
        #window.refresh()
